...
class Monitor:
    ...
    def _prediction_monitor(self):
        y_pred = self.model.predict(self.df_train_feat)
        self.df_train_feat["y_pred"] = y_pred
        df_ref_feat = self.df_train_feat[(self.df_train_feat.day_id.dt.year > YEAR_SPLIT)].copy()
        self.df_ref_feat = df_ref_feat.reset_index(drop=True)
        ...
        figure.write_html("tmp/dle_monitor.html")
    ...
    def _univariate_monitor(self):
        ...
        figure = univariate_data_drift.filter(period='all', metrics='jensen_shannon', column_names=['but_latitude']).plot(kind='distribution')
        figure.write_html("tmp/univariate_monitor.html")
